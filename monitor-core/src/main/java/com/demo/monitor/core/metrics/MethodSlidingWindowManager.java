...
public class MethodSlidingWindowManager {
    ...
    private static final ConcurrentHashMap<String, SlidingWindow> WINDOWS =
    ...
    public static void record(Span span) {
        ...
        SlidingWindow window =
                WINDOWS.computeIfAbsent(method, k -> new SlidingWindow());
    ...
    public static List<MethodMetric> topSlowMethods(int n) {
        ...
        return WINDOWS.entrySet()
                .stream()
                .map(e -> new MethodMetric(
                        e.getKey(),
                        e.getValue().avgLatency(),
                        e.getValue().errorRate(),
                        e.getValue().totalRequests()
                ))
                .sorted((a, b) ->
                        Double.compare(b.getAvgLatency(), a.getAvgLatency())
                )
                .limit(n)
                .toList();
    }
